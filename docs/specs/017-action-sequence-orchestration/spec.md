...
### FR13: 성능 모니터링 (Performance Monitoring)
- **FR13.1**: Action/Sequence/Task 실행 시간 추적
- **FR13.2**: 실행 시간 통계 제공 (min/max/avg/p95/p99 percentiles)
- **FR13.3**: 리소스 사용량 모니터링 (메모리, 스레드 풀 활용률)
- **FR13.4**: Periodic/Triggered Task 큐 깊이 모니터링
- **FR13.5**: 메트릭을 구조화된 형식으로 내보내기 (JSON/Prometheus)
- **FR13.6**: 리소스 한계에 대한 경고 임계값 설정 가능

### FR14: 실행 추적 및 진단 (Execution Tracing & Diagnostics)
- **FR14.1**: 실행 흐름 DAG(Directed Acyclic Graph) 생성
- **FR14.2**: Action 실행 순서를 보여주는 타임라인 시각화
- **FR14.3**: Sequence 실행의 크리티컬 패스 식별
- **FR14.4**: 실행 추적을 표준 형식으로 내보내기 (Chrome Trace Event Format)
- **FR14.5**: Health Check API로 시스템 상태 조회 가능
- **FR14.6**: 현재 실행 중인 Task 진단 API 제공
- **FR14.7**: 과거 실행 통계 조회 (최근 N회 실행)
...
6. **모니터링**:
   ...
   - Task 상태 실시간 조회 가능
   - 성능 메트릭 수집 및 내보내기 (Prometheus/JSON)
   - Health Check API로 시스템 상태 확인 가능
...
5. **Task**: 단일 Action 또는 Sequence를 포함하는 작업 관리 단위
6. **아키텍처**: Action → Sequence → Task 3계층 구조 (레거시 TaskManager 제거됨)
7. **Mission**: Task 완료 후 별도 구현 (이 사양에 포함되지 않음)
8. 동작들 간의 명시적 의존성만 지원 (암묵적 의존성 X)
9. 조건 평가는 동기식으로 수행 (비동기 조건 평가 X)
10. 병렬 동작 간 리소스 충돌은 개별 동작에서 처리
11. 시퀀스 정의는 정적이며, 실행 중 변경 불가능
12. 재시도는 실패한 개별 Action에만 적용
13. 타임아웃은 각 Action별로 적용
14. Task 주기적 실행은 별도 스레드에서 관리
15. 로그는 메모리 기반이며, 별도 영속성 계층과 통합 필요
16. **메트릭 수집**: 실행 시간 오버헤드 < 5%, 비동기 수집으로 성능 영향 최소화
17. **Health Check**: 외부 모니터링 시스템과의 통합을 위한 API 제공
